...
contract MysteryBox is IMysteryBox{
    ...
    */
    constructor(address _author, uint256 _max_quantity) public {
    ...
    */
    function generate(address _to, uint256 _counts, string memory _title, uint8 _class_id, uint8 _size) public onlyCaller {
        ...
        for (uint256 index = 0; index < _counts; index++) {
            ...
            bitfilxNFT.createNFT(_to, _title, _class_id, _size);

            attributes memory attr = attributes({owner_address: address(0), block_number:0, price:99*10**18, name:""}); // price is 99 USDT
    ...
    */
    function unBox(uint256[] memory _boxes) public {
        ...
        for (uint256 index = 0; index < _boxes.length; index++) {
            require(boxes_attributes[_boxes[index]].owner_address == msg.sender, "Not this box's owner");
    ...
    */
    function withdrawCash(uint256 _index) public onlyCaller {
    ...
    */
    function getAttributes(uint256 _index) public view returns(uint256 block_number, uint256 types, uint256 level, uint256 price, string memory name){
    ...
    */
    function changeName(uint256 _index, string memory _name) public {
        require(boxes_attributes[_index].owner_address == msg.sender, "Not this box's owner");
